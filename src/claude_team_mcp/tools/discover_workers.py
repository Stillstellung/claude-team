...
Provides discover_workers for finding existing Claude Code and Codex sessions in iTerm2.
"""
...
def register_tools(mcp: FastMCP, ensure_connection) -> None:
    ...
    async def discover_workers(
        ...
        """
        Discover existing Claude Code and Codex sessions running in iTerm2.

        For each iTerm2 pane, searches JSONL files in ~/.claude/projects/ and
        ~/.codex/sessions/ for matching iTerm session ID markers. Sessions spawned
        by claude-team write their iTerm session ID into the JSONL
        (e.g., <!claude-team-iterm:UUID!>), enabling reliable detection and
        recovery after MCP server restarts.
